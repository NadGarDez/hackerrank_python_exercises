...
def string_to_int(items):
    return map(lambda x: int(x), items)
...
A = string_to_int(filter(lambda x: len(x) > 0 , A.split()))
B = string_to_int(filter(lambda x: len(x) > 0 , B.split()))
product_list = map(lambda x: f'{x}',list(product(A,B)))
